...
export default function OneTimeCheckoutComplete() {
  const [searchParams] = useSearchParams();
  const navigate = useNavigate();
  const queryClient = useQueryClient();
  const [state, setState] = useState<State>("loading");
  const [invitation, setInvitation] = useState<OneTimeInvitation | null>(null);
  ...
  useEffect(() => {
    ...
    api<{ invitation?: OneTimeInvitation; message?: string }>(
      `/api/one-time-invitations/checkout/complete?session_id=${encodeURIComponent(sessionId)}`
    )
      .then((data) => {
        if (!data.invitation) {
          setState("pending");
      ...
      <div className="card p-8 text-center max-w-sm w-full animate-pop-in">
        ...
        {state === "pending" && (
          <>
            ...
            <p className="text-sm text-terracotta-400 mt-2">This can take a few seconds.</p>
